...
export function createRelayerConfig() {
  ...
  return {
    ...
    concurrency: Math.max(Number(process.env.MORPHEUS_RELAYER_CONCURRENCY || 4), 1),
    maxBlocksPerTick: Math.max(Number(process.env.MORPHEUS_RELAYER_MAX_BLOCKS_PER_TICK || 250), 1),
    maxRetries: Math.max(Number(process.env.MORPHEUS_RELAYER_MAX_RETRIES || 5), 0),
    retryBaseDelayMs: Math.max(Number(process.env.MORPHEUS_RELAYER_RETRY_BASE_DELAY_MS || 5000), 250),
    retryMaxDelayMs: Math.max(Number(process.env.MORPHEUS_RELAYER_RETRY_MAX_DELAY_MS || 300000), 1000),
    processedCacheSize: Math.max(Number(process.env.MORPHEUS_RELAYER_PROCESSED_CACHE_SIZE || 5000), 100),
    deadLetterLimit: Math.max(Number(process.env.MORPHEUS_RELAYER_DEAD_LETTER_LIMIT || 500), 10),
    logFormat: trimString(process.env.MORPHEUS_RELAYER_LOG_FORMAT || process.env.LOG_FORMAT || "json") || "json",
    logLevel: trimString(process.env.MORPHEUS_RELAYER_LOG_LEVEL || process.env.LOG_LEVEL || "info") || "info",
    confirmations: {
