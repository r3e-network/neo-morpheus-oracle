...
export default async function handler(request) {
  ...
  try {
    ...
    if (/\/feeds\/price\/.+/.test(path)) {
      return await handleFeedsPrice(
        decodeURIComponent(path.split('/').pop() || 'NEO-USD'),
        Object.fromEntries(url.searchParams.entries())
      );
    }
    if (path.endsWith('/feeds/price')) {
      return await handleFeedsPrice(url.searchParams.get('symbol') || payload.symbol || 'NEO-USD', {
        ...Object.fromEntries(url.searchParams.entries()),
        ...payload,
      });
    }
    ...
    if (path.endsWith('/oracle/feed') || payload.action === 'oracle_feed')
      return await handleOracleFeed(payload);
    if (path.endsWith('/txproxy/invoke')) return await handleTxProxyInvoke(payload);
    if (path.endsWith('/sign/payload') || payload.action === 'sign_payload')
      return await handleSignPayload(payload);
    if (path.endsWith('/relay/transaction') || payload.action === 'relay_transaction')
      return await handleRelayTransaction(payload);
