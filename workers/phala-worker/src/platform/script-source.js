...
function normalizeScriptReference(payload = {}) {
  const explicit = payload.script_ref && typeof payload.script_ref === "object" ? payload.script_ref : null;
  const contractHash = trimString(explicit?.contract_hash || payload.script_registry_contract || "");
  if (!contractHash) return null;
  const method = trimString(explicit?.method || payload.script_registry_method || "getScript") || "getScript";
  ...
  return {
    ...
    rpc_url: trimString(explicit?.rpc_url || payload.rpc_url || env("NEO_RPC_URL")),
    contract_hash: normalizeContractHash(contractHash),
    method: canonicalizeMethodName(method),
    params: args,
    encoding: trimString(explicit?.encoding || payload.script_registry_encoding || "utf8").toLowerCase() || "utf8",
  };
}
...
export async function resolveScriptSource(payload = {}) {
  ...
  const params = Array.isArray(reference.params)
    ...
    : [];
  const response = await rpcClient.invokeFunction(reference.contract_hash, reference.method, params);
