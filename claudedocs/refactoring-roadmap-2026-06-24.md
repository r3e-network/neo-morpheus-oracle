...
**Fix:** (a) skip the intermediate `processing` write when a terminal write follows in the same tick; (b) coalesce through the existing `createPersistor` trailing-write pattern (`queue.js:27-65` already coalesces *file* writes — extend to queue one trailing PATCH).
...
- `normalizePublicKey` ×3 (`attestation.js:9` returns `''` on invalid; `neo-n3.js:575` **throws**; `lib/neo-signers.js:210`) — *same name, opposite error contract.*
...
*All findings above are evidence-cited in the 5 domain audit transcripts. Read-only analysis; no source modified.*
