...
async function ensureFeeCredit(
  ...
  const currentCredit = BigInt(await invokeRead(rpcClient, oracleHash, "feeCreditOf", [{ type: "Hash160", value: creditRecipientHash }]) || "0");
  ...
  if (viaConsumer) {
    ...
    const signers = [new tx.Signer({ account: account.scriptHash, scopes: tx.WitnessScope.Global })];
    await consumer.invoke("depositOracleCredits", [sc.ContractParam.integer(deficit.toString())], signers);
  }

  const deadline = Date.now() + 60000;
  while (Date.now() < deadline) {
    const updatedCredit = BigInt(await invokeRead(rpcClient, oracleHash, "feeCreditOf", [{ type: "Hash160", value: creditRecipientHash }]) || "0");
...
async function main() {
  ...
  const networkConfig = JSON.parse(await fs.readFile(path.resolve(repoRoot, "config/networks/testnet.json"), "utf8"));
  ...
  const appId = trimString(networkConfig?.phala?.cvm_id || process.env.MORPHEUS_PAYMASTER_APP_ID || "");
  ...
  const buildPayload = (label) => JSON.stringify({
    trigger: {
      type: "interval",
      interval_ms: 300000,
  ...
  const registerOne = async (label) => {
    const txid = await consumer.invoke("requestRaw", [
      "automation_register",
      sc.ContractParam.byteArray(encodeUtf8Base64(buildPayload(label))),
    ], signers);
  ...
  try {
    ...
    registrationFeeStatus = await ensureFeeCredit(account, rpcUrl, networkMagic, rpcClient, oracleHash, requesterHash, 4, {
      viaConsumer: false,
  ...
  registrationA.callback = await waitForCallback(rpcClient, resolvedConsumerHash, registrationA.request_id, 180000);
  registrationB.callback = await waitForCallback(rpcClient, resolvedConsumerHash, registrationB.request_id, 180000);
  assertCondition(registrationA.callback?.success === true, "automation register callback should succeed for exhaustion-a");
  assertCondition(registrationB.callback?.success === true, "automation register callback should succeed for exhaustion-b");
  registrationA.automation_id = trimString(registrationA.callback.result_json?.result?.automation_id || registrationA.callback.result_json?.automation_id || registrationA.automation_id || "");
  registrationB.automation_id = trimString(registrationB.callback.result_json?.result?.automation_id || registrationB.callback.result_json?.automation_id || registrationB.automation_id || "");
  assertCondition(registrationA.automation_id, "automation register callback did not return automation_id for exhaustion-a");
  assertCondition(registrationB.automation_id, "automation register callback did not return automation_id for exhaustion-b");

  const pausedRelayerHandle = await stopRelayer({ appId, phalaApiToken });
  try {
    const sharedRequesterFeeStatus = await ensureFeeCredit(account, rpcUrl, networkMagic, rpcClient, oracleHash, sharedRequesterHash, 1, {
      viaConsumer: true,
      creditRecipientHash: sharedRequesterHash,
    });
    const creditBeforeQueue = BigInt(await invokeRead(rpcClient, oracleHash, "feeCreditOf", [{ type: "Hash160", value: sharedRequesterHash }]) || "0");
    assertCondition(creditBeforeQueue === requestFee, `expected exactly one request fee on the shared requester before queueing, got ${creditBeforeQueue}`);
    ...
    const recordA = await fetchAutomationRecord(supabaseUrl, serviceRoleKey, registrationA.automation_id, "testnet");
    const recordB = await fetchAutomationRecord(supabaseUrl, serviceRoleKey, registrationB.automation_id, "testnet");
    const queuedRuns = [...recordA.runs, ...recordB.runs].filter((item) => item.status === "queued");
    const failedRuns = [...recordA.runs, ...recordB.runs].filter((item) => item.status === "failed");

    assertCondition(queuedRuns.length === 1, `expected exactly one queued automation run under exhausted shared credit, got ${queuedRuns.length}`);
    assertCondition(failedRuns.length >= 1, "expected at least one failed automation run under exhausted shared credit");
    assertCondition(failedRuns.some((item) => /request fee not paid/i.test(String(item.error || ""))), "expected failed automation run to mention request fee not paid");

    const executionCountTotal = Number(recordA.job?.execution_count || 0) + Number(recordB.job?.execution_count || 0);
    assertCondition(executionCountTotal === 1, `expected total execution_count=1 across both jobs, got ${executionCountTotal}`);

    const creditAfterQueue = BigInt(await invokeRead(rpcClient, oracleHash, "feeCreditOf", [{ type: "Hash160", value: sharedRequesterHash }]) || "0");
    assertCondition(creditAfterQueue === 0n, `expected shared requester credit to be exhausted after one queue, got ${creditAfterQueue}`);
    ...
    const queuedCallback = await waitForCallback(rpcClient, resolvedConsumerHash, queuedChainRequestId, 180000);
    assertCondition(queuedCallback?.success === true, "the single funded automation execution should fulfill successfully");
