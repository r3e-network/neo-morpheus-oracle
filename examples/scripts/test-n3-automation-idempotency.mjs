...
async function startRelayer({ appId, phalaApiToken, pid = "" }) {
  ...
  await runRemoteDockerCommand("docker start morpheus-relayer >/dev/null", { appId, phalaApiToken });
}

async function waitForRelayerState({ appId, phalaApiToken, shouldBeRunning, pid = "", timeoutMs = 30000 }) {
  const startedAt = Date.now();
  while (Date.now() - startedAt < timeoutMs) {
...
async function main() {
  ...
  await waitForRelayerState({ appId, phalaApiToken, shouldBeRunning: false, pid: pausedRelayerPid });
  feeStatus = await ensureFeeCredit(account, rpcUrl, networkMagic, rpcClient, oracleHash, requesterHash, requiredAutomationCredits, {
    viaConsumer: false,
  ...
  const beforeRecord = await fetchAutomationRecord(supabaseUrl, serviceRoleKey, automationId, "testnet");
  ...
  try {
    ...
    const record = await fetchAutomationRecord(supabaseUrl, serviceRoleKey, automationId, "testnet");
    const queuedRuns = record.runs.filter((item) => item.status === "queued");
    const failedRuns = record.runs.filter((item) => item.status === "failed");
    assertCondition(record.job?.execution_count === 1, `expected execution_count=1 for ${automationId}, got ${record.job?.execution_count}`);
    assertCondition(trimString(record.job?.last_queued_request_id || "") === expectedQueuedRequestId, `expected last_queued_request_id=${expectedQueuedRequestId}, got ${record.job?.last_queued_request_id || ""}`);
    assertCondition(record.runs.length - beforeRunCount === 1, `expected exactly one new automation run for ${automationId}, got delta ${record.runs.length - beforeRunCount}`);
    assertCondition(queuedRuns.length === 1, `expected exactly one queued automation run, got ${queuedRuns.length}`);
    assertCondition(failedRuns.length === 0, `expected zero failed automation runs, got ${failedRuns.length}`);
    ...
    const queuedChainRequestId = queuedTxHash ? await waitForRequestId(rpcClient, queuedTxHash, 90000) : null;
    const queuedCallback = queuedChainRequestId ? await waitForCallback(rpcClient, consumerHash, queuedChainRequestId, 180000) : null;
