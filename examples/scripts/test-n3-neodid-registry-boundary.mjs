...
async function ensureRequestFeeCredit(account, rpcUrl, networkMagic, rpcClient, oracleHash, requiredRequests, callbackContract = "") {
  const requesterCredit = BigInt(await invokeRead(rpcClient, oracleHash, "feeCreditOf", [{ type: "Hash160", value: `0x${account.scriptHash}` }]) || "0");
  const callbackCredit = callbackContract
    ? BigInt(await invokeRead(rpcClient, oracleHash, "feeCreditOf", [{ type: "Hash160", value: callbackContract }]).catch(() => "0") || "0")
    : 0n;
  ...
  if (!transferVmState.includes("HALT")) {
    throw new Error(`request fee top-up failed: ${transfer.execution.exception || transferVmState}`);
  }
...
async function invokePersisted(contractHash, operation, params, { account, rpcUrl, networkMagic, signers }) {
...
async function testInvoke(contractHash, operation, params, { account, rpcUrl, networkMagic, signers }) {
  const contract = new experimental.SmartContract(contractHash, {
    rpcAddress: rpcUrl,
    networkMagic,
    account,
...
async function main() {
  ...
  assertCondition(await contractExists(rpcClient, resolvedConsumerHash), "configured testnet example consumer does not exist");
  ...
  assertCondition(actionTicket.verification_signature, "action ticket verification signature missing");
  assertCondition(actionTicket.verification_public_key, "action ticket verification public key missing");
  ...
  const vmState = String(mismatchPersisted.execution.vmstate || mismatchPersisted.execution.state || "");
  ...
  assertCondition(/invalid verification signature/i.test(exception), "ticket-consumption mismatch should fail with invalid verification signature");
