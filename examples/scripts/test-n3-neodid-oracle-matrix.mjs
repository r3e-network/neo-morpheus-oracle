...
const aaContractHash = normalizeHash160(process.env.AA_TESTNET_HASH || "0xe24d2980d17d2580ff4ee8dc5dddaa20e3caec38");
const aaVerifierHash = normalizeHash160(process.env.AA_RECOVERY_VERIFIER_TESTNET_HASH || "0x11d1012e071fac7fd75569981ac44da097913a84");
const vaultAccount = normalizeHash160(process.env.NEODID_TEST_VAULT_ACCOUNT || `0x${wallet.getScriptHashFromAddress("NTmHjwiadq4g3VHpJ5FQigQcD4fF5m8TyX")}`);
const disposableAccountA = normalizeHash160(process.env.NEODID_TEST_DISPOSABLE_A || "0x1111111111111111111111111111111111111111");
const disposableAccountB = normalizeHash160(process.env.NEODID_TEST_DISPOSABLE_B || "0x2222222222222222222222222222222222222222");
const newOwnerA = normalizeHash160(process.env.NEODID_TEST_NEW_OWNER_A || "0x3333333333333333333333333333333333333333");
const newOwnerB = normalizeHash160(process.env.NEODID_TEST_NEW_OWNER_B || "0x4444444444444444444444444444444444444444");
...
const feeStatus = await ensureRequestFeeCredit(account, rpcUrl, networkMagic, rpcClient, oracleHash, 8);

async function submitCase({ title, requestType, payload, expected, validate }) {
  const payloadText = JSON.stringify(payload);
  const txid = await consumer.invoke("requestRaw", [requestType, sc.ContractParam.byteArray(encodeUtf8Base64(payloadText))], signers);
  const requestId = await waitForRequestId(rpcClient, txid);
  const callback = await waitForCallback(rpcClient, resolvedConsumerHash, requestId, callbackTimeoutMs);
...
cases.push(await submitCase({
  ...
  validate({ callback }) {
    ...
    assertCondition(/^0x[0-9a-f]{64}$/.test(result.master_nullifier || ""), "bind_plain master_nullifier missing");
  },
}));
...
cases.push(await submitCase({
  ...
  validate({ callback, onchainRequest }) {
    ...
    assertCondition(/^0x[0-9a-f]{64}$/.test(result.master_nullifier || ""), "bind_encrypted master_nullifier missing");
    const payloadJson = onchainRequest?.payload_json || {};
    const payloadText = onchainRequest?.payload_text || "";
    assertCondition(typeof payloadJson.encrypted_params === "string" && payloadJson.encrypted_params.length > 0, "bind_encrypted missing encrypted_params on-chain");
    assertCondition(!payloadText.includes(bindEncryptedPatch.provider_uid), "bind_encrypted leaked provider_uid into on-chain payload");
  },
}));
...
const actionCaseA = await submitCase({
  ...
  validate({ callback }) {
    ...
    assertCondition(/^0x[0-9a-f]{64}$/.test(result.action_nullifier || ""), "action A action_nullifier missing");
  },
});
...

{
  ...
  assertCondition(actionA.action_nullifier === actionB.action_nullifier, "action ticket action_nullifier should stay stable across disposable accounts");
  assertCondition(actionA.digest !== actionB.digest, "action ticket digest should differ across disposable accounts");
}
...
const recoveryCaseA = await submitCase({
  ...
  validate({ callback }) {
    ...
    assertCondition(result.aa_contract?.toLowerCase() === aaContractHash.toLowerCase(), "recovery A aa_contract mismatch");
    assertCondition(result.new_owner?.toLowerCase() === newOwnerA.toLowerCase(), "recovery A new_owner mismatch");
    assertCondition(result.account_id === recoveryPayloadA.account_id, "recovery A account_id mismatch");
  },
});
...
const recoveryCaseB = await submitCase({
  ...
  validate({ callback }) {
    ...
    assertCondition(result.account_id === recoveryPayloadB.account_id, "recovery B account_id mismatch");
    assertCondition(result.new_owner?.toLowerCase() === newOwnerB.toLowerCase(), "recovery B new_owner mismatch");
  },
});
...

{
  ...
  assertCondition(recoveryA.action_id !== recoveryB.action_id, "recovery action_id should differ across account/owner targets");
  assertCondition(recoveryA.action_nullifier !== recoveryB.action_nullifier, "recovery action_nullifier should differ across account/owner targets");
}
...
cases.push(await submitCase({
  ...
  validate({ callback }) {
    assertCondition(callback?.success === false, "recovery failure case should return success=false");
    assertCondition(/new_owner/i.test(callback?.error_text || ""), "recovery failure callback did not surface the new_owner validation error");
  },
}));
