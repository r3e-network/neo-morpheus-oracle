...
function resolveExpectedUpdater() {
  const explicit = normalizeAddress(process.env.MORPHEUS_UPDATER_ADDRESS || process.env.MORPHEUS_UPDATER_EVM_ADDRESS || '');
...
const rpcUrl = trimString(process.env.NEOX_RPC_URL || process.env.NEO_X_RPC_URL || registry.neo_x?.rpc_url || '');
...
const checks = {
  registry_matches_oracle: normalizeAddress(registry.neo_x?.contracts?.morpheus_oracle_x || '') === oracleAddress,
  registry_matches_callback: normalizeAddress(registry.neo_x?.contracts?.oracle_callback_consumer_x || '') === callbackAddress,
