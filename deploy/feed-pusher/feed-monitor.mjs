...
const RPCS=(process.env.FEED_RPCS||'https://api.n3index.dev/mainnet,https://rpc10.n3.nspcc.ru:10331,https://mainnet1.neo.coz.io:443').split(',').map(s=>s.trim()).filter(Boolean);
...
const LOG=process.env.MONITOR_LOG||'/opt/morpheus/nitro/feed-monitor.log', STATUS=process.env.MONITOR_STATUS||'/opt/morpheus/nitro/feed-status.json';
...
const now=Math.floor(Date.now()/1000); const problems=[];
try{
  const j=await rpc('invokefunction',[FEED,'getLatest',[{type:'String',value:'TWELVEDATA:NEO-USD'}]]);
  ...
  const v=j.stack[0].value; const age=now-Number(v[3].value);
  ...
  writeFileSync(STATUS,JSON.stringify({ts:new Date().toISOString(),feed_age_min:Math.round(age/60),updater_gas:gas,ok:problems.length===0,problems},null,2));
  if(problems.length){ log('ALERT: '+problems.join(' | ')); process.exit(1); }
  log(`ok feed_age=${Math.round(age/60)}min gas=${gas.toFixed(1)}`);
}catch(e){ log('monitor RPC error: '+(e.message||e)); process.exit(2); }
