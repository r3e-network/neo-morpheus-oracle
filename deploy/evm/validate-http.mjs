...
(async () => {
  const payload = ethers.toUtf8Bytes(
    JSON.stringify({ url: 'https://api.coinbase.com/v2/prices/ETH-USD/spot', method: 'GET', json_path: 'data.amount' })
  );
  ...
  let id;
  for (const lg of rcpt.logs) { try { const p = c.interface.parseLog(lg); if (p?.name === 'RequestQueued') id = p.args.requestId; } catch {} }
  console.log(`HTTP request id ${id} queued (block ${rcpt.blockNumber}) — waiting for relayer + worker...`);
  ...
  for (let i = 0; i < 40; i++) {
    ...
    let text = '';
    try { text = ethers.toUtf8String(r.result).replace(/[^\x20-\x7e]/g, ''); } catch {}
