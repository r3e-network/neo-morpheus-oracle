...
(async () => {
  ...
  let id;
  for (const lg of rcpt.logs) { try { const p = c.interface.parseLog(lg); if (p && p.name === 'RequestQueued') id = p.args.requestId; } catch {} }
  ...
  for (let i = 0; i < 40; i++) {
    ...
    if (st === 1) { console.log(`  [t+${(i + 1) * 5}s] pending`); continue; }
    ...
    if (st === 2) {
      ...
      const enc = ethers.AbiCoder.defaultAbiCoder().encode(
        ...
        ['morpheus-evm-fulfillment-v1', CHAIN_ID, ADDR, id,
          ethers.keccak256(ethers.toUtf8Bytes(r.appId)), ethers.keccak256(ethers.toUtf8Bytes(r.moduleId)),
          ethers.keccak256(ethers.toUtf8Bytes(r.operation)), true, ethers.keccak256(r.result), ethers.keccak256(ethers.toUtf8Bytes(r.error))],
      );
    ...
    } else { console.log('⚠️ fulfilled as Failed:', r.error); }
    return;
  }
