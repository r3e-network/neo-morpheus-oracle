...
describe('MorpheusOracleX', function () {
  ...
  it('rejects oversized request metadata and oracle key inputs', async function () {
    ...
    await expect(oracle.setOracleEncryptionKey('a'.repeat(65), 'pk')).to.be.revertedWith(
      'algorithm too long'
    );
    await expect(oracle.setOracleEncryptionKey('RSA', 'p'.repeat(2049))).to.be.revertedWith(
      'public key too long'
    );
    await expect(
      oracle
        .connect(requester)
        .request('x'.repeat(65), '0x1234', await consumer.getAddress(), 'onOracleResult', {
          value: requestFee,
        })
    ...
    await expect(
      oracle
        .connect(requester)
        .request('oracle', '0x1234', await consumer.getAddress(), 'm'.repeat(65), {
          value: requestFee,
        })
  ...
  it('requires the exact request fee and allows fee withdrawal', async function () {
    ...
    await expect(
      oracle
        .connect(requester)
        .request('oracle', '0x1234', await consumer.getAddress(), 'onOracleResult')
    ...
    await expect(oracle.withdrawAccruedFees(ethers.ZeroAddress, requestFee)).to.be.revertedWith(
      'invalid recipient'
    );
  ...
  it('supports prepaid fee credits and updater-queued automation requests', async function () {
    ...
    await oracle
      .connect(requester)
      .depositFeeCredit(await requester.getAddress(), { value: requestFee });
