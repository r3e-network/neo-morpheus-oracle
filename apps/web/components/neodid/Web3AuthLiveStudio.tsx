...
const DEFAULT_WEB3AUTH_CLIENT_ID =
  'BHpd11oKRDW7--gsLg_lcAbCC9tiAjcDfMlYFabfVipkPe9I2enmxNzY1TfqalImrPEntXCpf6Gkl4N23rBqTS0';
...
function Web3AuthLiveStudioInner({ originDataState }: { originDataState: OriginDataState | null }) {
  ...
  async function handleBindProbe() {
    ...
    try {
      ...
      if (encryptToken) {
        ...
        payload.encrypted_params = await encryptJsonWithOraclePublicKey(
          keyBody.public_key,
          JSON.stringify({
            id_token: nextToken,
          })
  ...
  return (
    <div className="fade-in">
      ...
      >
        Sign in with Web3Auth, fetch a real <code>id_token</code>, optionally seal it locally with
        Morpheus X25519 encryption, then submit a live
        <code>neodid_bind</code> request directly to the production NeoDID TEE or generate a short{' '}
        <code>encrypted_params_ref</code> for the on-chain Oracle callback flow.
      </p>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontWeight: 700 }}>
              {audienceConfigured ? (
                ...
                <ShieldAlert size={18} color="#ff7b7b" />
              )}
              <span>
                {audienceConfigured ? 'Audience Verified In TEE' : 'Audience Not Configured'}
              </span>
            </div>
            <div style={{ color: 'var(--text-secondary)', marginTop: '0.65rem', lineHeight: 1.6 }}>
              App id: <code>{runtime?.app_id || '...'}</code>
              <br />
              Compose hash: <code>{runtime?.compose_hash || '...'}</code>
              <br />
          ...
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            ...
            {runtime?.verification_public_key ? (
              <button
                ...
                onClick={() =>
                  copyWithToast('runtime-pubkey', runtime.verification_public_key || '')
                }
              >
                <Copy size={14} />{' '}
                {copiedItem === 'runtime-pubkey' ? 'Copied' : 'Copy Verifier Key'}
              </button>
            ) : null}
          </div>
        </div>
      ...
      <div className="grid grid-2" style={{ gap: '1.5rem', marginBottom: '1.5rem' }}>
        <div className="card-industrial" style={{ padding: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
            ...
            <button
              className="btn-secondary"
              onClick={() => void refreshIdentityArtifacts()}
              disabled={!isConnected || busy}
            >
          ...
          {jwtPayload ? (
            <div style={{ marginTop: '1rem', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
              Subject: <code>{String(jwtPayload.sub || 'unknown')}</code>
              <br />
              Email: <code>{String(jwtPayload.email || jwtPayload.verifierId || 'n/a')}</code>
              <br />
              Verifier:{' '}
              <code>{String(jwtPayload.verifier || jwtPayload.aggregateVerifier || 'n/a')}</code>
            </div>
          ) : null}
          ...
          <div
            style={{
              marginTop: '1rem',
              color: originDataState?.error ? '#ff8f8f' : 'var(--text-secondary)',
              lineHeight: 1.6,
            }}
          >
            Origin signing:{' '}
            <code>
              {originDataState?.origin ||
                currentOrigin ||
            ...
            {originDataState?.error ? (
              <>
                <br />
                Reason: <code>{describeWeb3AuthError(originDataState.error)}</code>
              </>
            ) : null}
          </div>
        </div>
        ...
        <div className="card-industrial" style={{ padding: '1.5rem' }}>
          ...
          <div style={{ color: 'var(--text-secondary)', lineHeight: 1.6, marginBottom: '1rem' }}>
            Raw token: <code>{identityToken ? maskToken(identityToken) : 'not loaded'}</code>
          </div>

          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
            <button
              className="btn-secondary"
              onClick={() => copyWithToast('id-token', identityToken)}
          ...
          {jwtPayload ? (
            <div style={{ color: 'var(--text-secondary)', lineHeight: 1.6 }}>
              Subject: <code>{String(jwtPayload.sub || 'n/a')}</code>
              <br />
              Email: <code>{String(jwtPayload.email || jwtPayload.verifierId || 'n/a')}</code>
              <br />
              Audience: <code>{String(jwtPayload.aud || 'n/a')}</code>
              <br />
              Aggregated root:{' '}
              <code>
                {String(jwtPayload.aggregateVerifierId || jwtPayload.verifierId || 'n/a')}
              </code>
            </div>
          ) : (
      ...
      <div className="card-industrial" style={{ padding: '1.5rem', marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
          <button
            className="btn-ata"
            onClick={() => void handleBindProbe()}
            disabled={!isConnected || !identityToken || bindLoading}
          >
        ...
        <div style={{ color: 'var(--text-secondary)', lineHeight: 1.7 }}>
          This page sends the token directly to the NeoDID worker, or hides it inside{' '}
          <code>encrypted_params</code> when the encryption toggle is enabled. The worker verifies
          the JWT against Web3Auth JWKS inside the TEE and derives the stable provider root there.
        </div>
        ...
        {bindResult ? (
          ...
          >
            ...
            >
              Mode: <code>{String(bindResult.mode || 'n/a')}</code>
              <br />
              Provider: <code>{String(bindResult.provider || 'n/a')}</code>
              <br />
              Master nullifier: <code>{String(bindResult.master_nullifier || 'n/a')}</code>
              <br />
              Attestation hash: <code>{String(bindResult.attestation_hash || 'n/a')}</code>
              <br />
            ...
            <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
              ...
              <button
                className="btn-secondary"
                onClick={() => copyWithToast('bind-json', JSON.stringify(bindResult, null, 2))}
              >
      ...
      <div className="card-industrial" style={{ padding: '1.5rem', marginBottom: '1.5rem' }}>
        ...
        <p style={{ color: 'var(--text-secondary)', lineHeight: 1.7, marginTop: 0 }}>
          Large Web3Auth JWTs are too big to place directly into a Neo N3 Oracle request event. Use
          this step to store the sealed ciphertext in Supabase and carry only{' '}
          <code>encrypted_params_ref</code> on-chain. The worker resolves the ciphertext by
          reference, decrypts it inside the TEE, verifies the JWT, and returns the normal callback
          envelope.
        </p>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
          <button
            className="btn-ata"
            onClick={() => void handleCreateSecretRef()}
            disabled={!isConnected || !identityToken || storeRefLoading}
          >
          ...
          {secretRefResult?.secret_ref ? (
            <button
              className="btn-secondary"
              onClick={() => copyWithToast('secret-ref', secretRefResult.secret_ref || '')}
            >
          ...
          {oracleRefPayload ? (
            <button
              className="btn-secondary"
              onClick={() => copyWithToast('oracle-ref-payload', oracleRefPayload)}
            >
              <Copy size={14} />{' '}
              {copiedItem === 'oracle-ref-payload' ? 'Copied' : 'Copy Oracle payload'}
            </button>
          ) : null}
        </div>

        {secretRefError ? (
          <div style={{ marginBottom: '1rem', color: '#ff8f8f', lineHeight: 1.6 }}>
            {secretRefError}
          </div>
        ) : null}
        ...
        {secretRefResult?.secret_ref ? (
          <>
            <div style={{ color: 'var(--text-secondary)', lineHeight: 1.7, marginBottom: '1rem' }}>
              Secret ref: <code>{secretRefResult.secret_ref}</code>
              <br />
              Target chain: <code>{secretRefResult.target_chain || 'neo_n3'}</code>
              <br />
              Encryption:{' '}
              <code>
                {secretRefResult.encryption_algorithm || 'X25519-HKDF-SHA256-AES-256-GCM'}
              </code>
            </div>
...
export function Web3AuthLiveStudio() {
  ...
  const web3AuthNetwork = (process.env.NEXT_PUBLIC_WEB3AUTH_NETWORK ||
    DEFAULT_WEB3AUTH_NETWORK) as 'sapphire_mainnet';
    ...
    void (async () => {
      try {
        const response = await fetch(
          `/api/web3auth/origin-data?origin=${encodeURIComponent(window.location.origin)}`
        );
        const body = await response.json().catch(() => ({}));
        if (!response.ok)
          throw new Error(body?.error || 'Failed to load Web3Auth origin signature');
