...
import { ClipboardList, ArrowRight, Shield, Cpu, Lock, Globe, Terminal } from "lucide-react";
  ...
  return (
    <div className="fade-in">
      ...
      <p className="lead" style={{ fontSize: "1.1rem", color: "var(--text-primary)", marginBottom: "2.5rem", lineHeight: 1.6 }}>
        These are copy-ready payloads and zero-code testing patterns for users who want to participate immediately.
        Pick a template, encrypt sensitive fields if needed, submit it through the Oracle contract, and read the callback.
      </p>
      ...
      <div className="card-industrial" style={{ padding: "2rem", borderLeft: "4px solid var(--neo-green)", marginBottom: "2.5rem" }}>
        ...
        <p style={{ color: "var(--text-secondary)", lineHeight: 1.7, marginBottom: 0 }}>
          Call <code>request(requestType, payload, callbackContract, callbackMethod)</code> with callback target <code>{universalConsumer}</code>,
          callback method <code>onOracleResult</code>, and attach exactly <code>0.01 GAS</code>.
        </p>
      </div>

      <h2>1. Privacy Oracle Templates</h2>
      <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem", margin: "2rem 0 3rem" }}>
        {oracleTemplates.map((item) => (
          <div key={item.title} className="card-industrial" style={{ padding: "1.75rem" }}>
            ...
            <p style={{ marginTop: 0, color: "var(--text-secondary)", lineHeight: 1.6 }}>{item.desc}</p>
      ...
      <div className="grid grid-2" style={{ gap: "1.5rem", marginTop: "2rem" }}>
        ...
        <Link href="/docs/quickstart" className="card-industrial" style={{ padding: "2rem", textDecoration: "none" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
