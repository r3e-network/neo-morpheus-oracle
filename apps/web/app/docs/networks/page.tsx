...
type NetworkKey = keyof typeof networkRegistry;
    ...
    <div className="fade-in">
      ...
      <div className="card-industrial" style={{ padding: "1.5rem", borderLeft: "4px solid var(--neo-green)", marginBottom: "2rem" }}>
        <p style={{ margin: 0, color: "var(--text-secondary)", lineHeight: 1.7 }}>
          Source of truth:
          {" "}
          <code>config/networks/mainnet.json</code>
          {" "}
          and
          {" "}
          <code>config/networks/testnet.json</code>.
          {" "}
          The Phala launcher ids live in
          {" "}
          <code>phala.mainnet.toml</code>
          {" "}
          and
          {" "}
          <code>phala.testnet.toml</code>.
        </p>
      </div>

      <div style={{ border: "1px solid var(--border-dim)", borderRadius: "4px", overflow: "hidden", marginBottom: "2rem", background: "#000" }}>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.label} style={{ borderBottom: "1px solid var(--border-dim)" }}>
                ...
                <td style={{ padding: "1rem 1.25rem", fontFamily: "var(--font-mono)", fontSize: "0.78rem" }}>{renderValue(row.mainnet)}</td>
                <td style={{ padding: "1rem 1.25rem", fontFamily: "var(--font-mono)", fontSize: "0.78rem" }}>{renderValue(row.testnet)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="card-industrial" style={{ padding: "1.5rem", borderLeft: "4px solid var(--neo-green)", marginBottom: "2rem" }}>
        <p style={{ margin: 0, color: "var(--text-secondary)", lineHeight: 1.7 }}>
          Neo N3 remains the active supported runtime path. The live request fee is <code>0.01 GAS</code>, the confidential payload transport is <code>X25519-HKDF-SHA256-AES-256-GCM</code>, and the public NeoDID service DID is <code>did:morpheus:neo_n3:service:neodid</code>.
        </p>
      </div>
