...
async function requestDidJson(did: string, format: "resolution" | "document") {
  const response = await fetch(`/api/neodid/resolve?did=${encodeURIComponent(did)}${format === "document" ? "&format=document" : ""}`, {
    cache: "no-store",
  });
  ...
  return (
    <div className="min-h-screen bg-main flex flex-col">
      <nav className="navbar" style={{ position: "sticky" }}>
        <Link href="/launchpad" className="nav-logo" style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <ArrowLeft size={18} />
          <span className="text-gradient" style={{ letterSpacing: "0.1em" }}>
            MORPHEUS <span style={{ color: "var(--text-dim)", fontWeight: 400 }}>| NeoDID Resolver</span>
          </span>
        </Link>
        <div className="nav-links">
          <Link href="/docs/neodid" className="nav-link">NeoDID Docs</Link>
          <Link href="/docs/r/NEODID_DID_METHOD" className="nav-link">DID Spec</Link>
        </div>
      </nav>
      ...
      <main className="container" style={{ flex: 1, padding: "2rem 0 4rem" }}>
        <div className="fade-in">
          ...
          <h1 style={{ fontSize: "clamp(2.3rem, 5vw, 3.1rem)", marginBottom: "1rem" }}>NeoDID Resolver</h1>
          <p className="lead" style={{ maxWidth: "860px", lineHeight: 1.7, color: "var(--text-secondary)", marginBottom: "2rem" }}>
            Resolve the public W3C DID document for the Morpheus NeoDID service, a vault subject namespace, or an AA recovery namespace.
            The resolver intentionally exposes service metadata and verifier material only. Provider UIDs, master nullifiers, encrypted params, and action tickets remain private.
          </p>

          <div className="card-industrial" style={{ padding: "1.5rem", marginBottom: "1.5rem" }}>
            <div style={{ display: "grid", gap: "1rem" }}>
              <div>
                <div style={{ fontSize: "0.72rem", marginBottom: "0.5rem", color: "var(--text-muted)", fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.08em", fontFamily: "var(--font-mono)" }}>
                  DID
                </div>
                <input
                  value={did}
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.75rem" }}>
                <button className={`btn ${format === "resolution" ? "btn-primary" : "btn-secondary"}`} onClick={() => setFormat("resolution")}>
                  Resolution Object
                </button>
                <button className={`btn ${format === "document" ? "btn-primary" : "btn-secondary"}`} onClick={() => setFormat("document")}>
                  DID Document
                </button>
          ...
          {error ? (
            <div className="card-industrial" style={{ padding: "1rem 1.25rem", borderLeft: "4px solid #ff7b72", marginBottom: "1.5rem" }}>
          ...
          {result ? (
            <CodeBlock
              language="json"
              title={format === "document" ? "application/did+ld+json" : 'application/ld+json;profile="https://w3id.org/did-resolution"'}
              code={JSON.stringify(result, null, 2)}
            />
          ) : null}

          <div className="grid grid-2" style={{ gap: "1.25rem", marginTop: "2rem" }}>
            <Link href="/docs/neodid" className="card-industrial" style={{ padding: "1.5rem", textDecoration: "none" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontWeight: 800, fontSize: "1rem", color: "#fff" }}>NeoDID Docs</span>
                <ExternalLink size={18} color="var(--neo-green)" />
              </div>
              <p style={{ color: "var(--text-secondary)", marginTop: "0.85rem", marginBottom: 0 }}>
                Read the bind, action-ticket, recovery-ticket, Web3Auth, and DID method model in one place.
              </p>
            </Link>
            <Link href="/docs/r/NEODID_DID_METHOD" className="card-industrial" style={{ padding: "1.5rem", textDecoration: "none" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
