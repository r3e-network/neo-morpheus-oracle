...
export async function POST(request: Request) {
  ...
  const automationMod = (await import(automationModulePath)) as {
    processAutomationJobs: (config: unknown, logger: unknown, deps?: Record<string, unknown>) => Promise<unknown>;
  };
  ...
  const persistenceMod = (await import(persistenceModulePath)) as {
    fetchAutomationJobById: (automationId: string) => Promise<{ automation_id?: string; status?: string } | null>;
    fetchActiveAutomationJobs: (limit?: number, dueAtIso?: string | null) => Promise<Array<{ automation_id?: string; status?: string }>>;
  };
  ...
  const result = await withMorpheusNetworkContext(network, async () => {
    ...
    const summary = await automationMod.processAutomationJobs(config, logger, {
      fetchActiveAutomationJobs: async () => {
        ...
        return jobs.filter((job: { automation_id?: string }) => trimString(job.automation_id) === automationId);
      },
    });
