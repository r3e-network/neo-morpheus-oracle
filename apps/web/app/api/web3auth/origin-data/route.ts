...
export async function GET(request: NextRequest) {
  const candidateOrigin =
    trimString(request.nextUrl.searchParams.get('origin')) ||
  ...
  const clientId = trimString(
    process.env.WEB3AUTH_CLIENT_ID || process.env.NEXT_PUBLIC_WEB3AUTH_CLIENT_ID
  );
  const clientSecret = trimString(
    process.env.WEB3AUTH_CLIENT_SECRET || process.env.WEB3AUTH_APP_KEY
  );
